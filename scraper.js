/**
 * SonarGold BD — Price Scraper v3.0 (Datacenter-IP Fix)
 * ============================================================
 ...
 *
 * KEY FIXES IN v3.0:
 *   1. Uses Cloudflare WARP proxy (residential IP) when WARP_PROXY env is set
 *   2. Tries multiple scraping strategies: direct fetch → curl-impersonate → puppeteer
 *   3. Falls back to Wayback Machine (web.archive.org) snapshot if live site is blocked
 *   4. Caches last successful BAJUS prices and reuses them if scraping fails today
 *   5. Stale-price guard: won't reuse cached prices older than 3 days
 *
...
async function scrapeBajus() {
  info('Starting BAJUS scrape (v3.0 — multi-strategy)…');
...
async function main() {
  ...
  info('══════════════════════════════════════════════');
  info('SonarGold Scraper v3.0 (Datacenter-IP Fix) starting…');
